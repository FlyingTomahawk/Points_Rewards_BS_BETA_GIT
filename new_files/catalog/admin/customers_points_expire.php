...
if ((USE_POINTS_SYSTEM == 'true') && tep_not_null(POINTS_AUTO_EXPIRES)){
  ...
  if (tep_not_null(POINTS_EXPIRES_REMIND)){
    ...
    $customer_query = tep_db_query("select customers_gender, customers_lastname, customers_firstname, customers_email_address, customers_shopping_points, customers_points_expires from customers where (CURDATE() + '". (int)POINTS_EXPIRES_REMIND ."') = customers_points_expires");
